...
export default function UserPlan() {
  ...
  const onCloseModal = () => {
    dispatch({
      type: "SHOW_MODAL",
      payload: { title: "", open: false, content: null },
    });
  };
  ...
  const handleClick = (e) => {
        ...
        <div>
          {e.name ? (
            <Rating
              ...
              readOnly
              name="customized-10"
              defaultValue={(usersPerPlan * stars) / allUsers}
              max={stars}
            />
          ) : (
            <Skeleton />
    ...
    dispatch({
      type: "SHOW_MODAL",
      payload: { open: true, title: e.name, content: <InfoCard /> },
    });
  };
  return (
    <>
